/* ==========================================================================
   ...
   Este arquivo não conhece o app.js: ele escuta eventos de DOM (`urbana:evento`)
...
(() => {
  ...
  const CHAVE_SESSAO = 'urbana:sessao';
  const MINUTOS_SESSAO = 30;

  const consentimento = () =>
    window.URBANA_CONSENTIMENTO?.estado() || { decidido: false, analytics: true, ad: false };

  /* ── Identidade ───────────────────────────────────────────────────────
     O id do VISITANTE é criado pelo servidor (cookie `_urb`), porque cookie
  ...
  /* ── Escuta ─────────────────────────────────────────────────────────
     O app.js dispara `urbana:evento`; aqui só repassamos. Um clique genérico
     de WhatsApp que o app não cubra ainda é capturado por delegação. */

  function escutar() {
    addEventListener('urbana:evento', (e) => {
  ...
  function iniciar() {
    ...
    addEventListener('urbana:consentimento', (e) => {
      if (e.detail?.aceitou && !jaContavaAnuncio) evento('page_view', { tardio: true });
    });
  }

  window.URBANA_TRACK = { evento };
